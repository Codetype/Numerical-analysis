...
//correct solution
int correct_gauss(int n, float ** AB, float * X, int * W)
{
  int   i,j,k;
  float m,s;
...
//testing
void test(float** AB, float* X, int* W, float* R, int n){
    if(!correct_gauss(n, AB, X, W)) exit(1);
